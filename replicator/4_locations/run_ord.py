...
for port in ins:
    if args.begin_mode == "lower_left":
        updatePin(port, 0, 0)
    elif args.begin_mode == "upper_left":
        updatePin(port, 0, core_ury)
    elif args.begin_mode == "upper_right":
        updatePin(port, core_urx, core_ury)
    else:
        raise Exception("Unknown begin mode")

for port in outs:
    if args.end_mode == "lower_left":
        updatePin(port, 0, 0)
    elif args.end_mode == "upper_right":
        updatePin(port, core_urx, core_ury)
    else:
        raise Exception("Unknown end mode")
...
setup_cmd = f'set_dft_config -chain_count {args.k} -max_imbalance 2 -scanopt_time_limit 300 -scan_order_constraints_file "{args.output}/constraints" -scan_order_metric PLACEMENT -scan_enable_name_pattern "scan_enable_{{}}" -clock_mixing no_mix'
...
chainlib.graph(block, S, f"{args.output}/plot.png")
