...
dff_map = {
    "sky130_fd_sc_hd__sedfxtp_1": "sky130_fd_sc_hd__sdfbbn_1",
    "sky130_fd_sc_hd__sedfxtp_4": "sky130_fd_sc_hd__sdfbbn_2",
    "sky130_fd_sc_hd__sdfrbp_1": "sky130_fd_sc_hd__sdfbbn_1",
    "sky130_fd_sc_hd__sdfrbp_2": "sky130_fd_sc_hd__sdfbbn_2",
    "sky130_fd_sc_hd__sdfxtp_1": "sky130_fd_sc_hd__sdfrtn_1",
    "sky130_fd_sc_hd__sdfsbp_2": "sky130_fd_sc_hd__sdfbbn_2"
}

flops_to_neg = []
if args.polarity_mode == "split":
    flops_to_neg = sorted(scan_flops, key=lambda i: i.getBBox().xMin())[:n_flops // 2]
elif args.polarity_mode == "even":
    # Deterministic option:
    #flops_to_neg = sorted(scan_flops, key=lambda i: i.getBBox().xMin())[::n_flops // 2]
    flops_to_neg = random.sample(scan_flops, n_flops // 2)
else:
    raise Exception("Unknown Polarity Mode")
for flop in flops_to_neg:
    m = flop.getMaster().getName()
    n = flop.getName()
    
    if m in dff_map and library.findMaster(dff_map[m]) is not None:
        inst = odb.dbInst_create(block, library.findMaster(dff_map[m]), n+"__NEG")
        for it in inst.getITerms():
            orig_it = flop.findITerm(it.getMTerm().getName())
            if orig_it is not None and orig_it.getNet() is not None:
                it.connect(orig_it.getNet())
                
        inst.findITerm("CLK_N").connect(flop.findITerm("CLK").getNet())
        
        inst.setOrient(flop.getOrient())
        inst.setLocation(*flop.getLocation())
...
setup_cmd = f'set_dft_config -chain_count {args.k} -polarity_mode strict -scanopt_time_limit 300 -max_imbalance 2 -scan_order_constraints_file "../constraints/chains{args.k}" -scan_order_metric PLACEMENT -scan_enable_name_pattern "scan_enable_{{}}" -clock_mixing no_mix'
...
chainlib.graph(block, S, f"{args.output}/plot.png", "__NEG", "negatively polarized")