...
def main() -> int:
    ...
    validate_jobs = max(1, int(os.environ.get("SURROGATE_VALIDATE_JOBS", "10"))) if validate_enabled else 1
    ...
    def make_args_for(variant: str, sdc_file: Path, synth_netlist: Optional[Path], params: Dict[str, Any]) -> List[str]:
        ...
        if synth_netlist is not None:
            args.append(f"SYNTH_NETLIST_FILES={synth_netlist}")
        ...
        if route_prefilter:
            ...
            if not reused_route_jobs:
                ...
                for idx, c in enumerate(route_cands, start=1):
                    ...
                    tune_results = _results_dir(flow_dir, platform, design, c.tune_variant)
                    synth_netlist = tune_results / "1_synth.v"
